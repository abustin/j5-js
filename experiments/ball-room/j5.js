...
var j5 = (function() {
    ...
    var createSphereData = function(latitudeBands, longitudeBands, radius) {
        ...
        for (latNumber = 0; latNumber <= latitudeBands; latNumber++) {
            ...
            for (var longNumber = 0; longNumber <= longitudeBands; longNumber++) {
                ...
                comboData.push(radius * x);
                comboData.push(radius * y);
                comboData.push(radius * z);
                comboData.push(1);
        ...
        //mat.translate(radius,radius,-radius);
        //offsetVerts(mat,comboData,3,5,0);
        ...
        return {
            vertices: createBuffer(gl, gl.ARRAY_BUFFER,         new Float32Array(comboData),8),
	        indices:  createBuffer(gl, gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(indexData), 1),
    ...
    var j5helper = function(name) {
        
        switch(name) {
            case "gl": return gl;
            case "createBuffer": return createBuffer;
            case "defaultCamera": return defaultCamera;
        }
	...
	var api = {
	    ...
	    extrude: function(shape2d, scale) {
	        
	        
	        var geom = shape2d.getData();
	        var polygons = shape2d.getPolygons();
	        
	        var vertices = geom.vertices.concat();
	        var indices = geom.indices.concat()
	        var len = geom.vertices.length/5;
	        
	        
	        // create back face
	        var mat = new CanvasMatrix4();
	        mat.translate(0,0,-scale);
	        
	        vertices = vertices.concat(offsetVerts(mat,geom.vertices.concat(),3,5,0));
	        
	        indices = indices.concat(geom.indices.map(function(idx) {
                return len + idx;
            }));
            ...
            polygons.forEach(function(polygon) {
                ...
                var firstId = polygon[0].id;
                //var firstpoint = polygon[];
                
                indices.push(idx);
                indices.push(idx+len);
                indices.push(firstId+len);
                
                indices.push(firstId+len);
                indices.push(firstId);
                indices.push(idx);
            });
	    ...
	    registerComponent: function(name, fn){
	    ...
	    sphere: function(latBands,longBands,radius) {
	        
	        latBands = latBands || 30;
	        longBands = longBands || 30;
	        radius = radius || .5;
	        
	      
            var geomId = "sphere,"+[latBands,longBands,radius];
	        var geomData = geomCache[geomId] = geomCache[geomId] || createSphereData(latBands,longBands,radius);
	        ...
	        return {
			    draw: function(shader, position, camera) {
    				...
    				shader.setAttribute(gl, "aTextureCoord",   8*4, 4*4);
    				shader.setAttribute(gl, "aVertexPosition", 8*4, 0*4);
